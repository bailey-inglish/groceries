...
export async function POST(req: NextRequest) {
  try {
    ...
    const user = await prisma.user.findUnique({
      ...
      select: { dietaryRestrictions: true, householdSize: true },
    })
    ...
    if (!apiKey) {
      return NextResponse.json(
        { status: "error", message: "Gemini API key not configured in environment variables.", recipes: [], missingIngredients: [] },
        { status: 400 }
      )
    }
    ...
    const generation = await generateRecipeSuggestions({
      apiKey,
    ...
    if (generation.status !== "success" || generation.recipes.length === 0) {
    ...
    const recipesWithCoverage = generation.recipes.map((s) => {
    ...
    recipesWithCoverage.sort((a, b) => b.coverageScore - a.coverageScore)

    if (parsed.data.save) {
      await Promise.all(
        recipesWithCoverage.map((s) =>
    ...
    return NextResponse.json({
      ...generation,
      recipes: recipesWithCoverage,
      suggestions: recipesWithCoverage,
    })
  } catch (error) {
    ...
    return NextResponse.json(
      {
        status: "error",
        message: "Could not generate meal ideas right now.",
        recipes: [],
        missingIngredients: [],
      },
      { status: 422 }
